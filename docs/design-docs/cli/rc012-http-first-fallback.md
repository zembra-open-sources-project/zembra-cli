...
| HTTP 回退提示 | 通过 stderr 打印一次用户可见提示，避免污染 JSON stdout |
| 无 `http_base_url` | 直接打开 direct repository |
| direct 所需 `database.path` 或 `workspace.id` 缺失 | 沿用现有配置错误输出 |

HTTP 优先包装器只包装现有 repository 方法，不改命令层 stdout 输出逻辑。direct repository 延迟到第一次回退时打开，并在 CLI context 退出时统一关闭连接。首次回退时向 stderr 打印一行提示，说明 HTTP backend 不可用并展示本地数据库路径；同一个 repository 实例后续回退不重复打印。
...
| CLI 提示测试 | HTTP 回退时 stderr 包含本地数据库回退提示 |
