...
- [rc013] (85075b0) : 2026.06.26 完成 docs/exec-plans/active/cli/rc013-workspace-commands.md 对应开发，新增 workspaces list 与 set-default 命令，列表从配置级联读取后端地址并支持表格和 JSON 输出，设置默认 workspace 写入 config.cli.toml；后续修正全局 server 配置回退和 latest note 人类可读时间展示。
