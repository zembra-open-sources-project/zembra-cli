...
功能：新增 workspace 命令专用配置读取函数，读取配置中的后端地址，并把 CLI 配置中的 workspace 字段作为可选值返回；新增 `write_default_workspace()`，只更新 `~/.zembra/config.cli.toml` 的 `[workspace]` 段。

实现说明：读取函数不能写死默认 URL，优先读取级联合并后的 `cli.http_base_url`，缺失时使用配置中的 `server.host` 和 `server.port` 组装 URL，缺少可用后端地址时返回明确配置错误；读取函数允许缺少 `workspace.id`，用于支持首次通过 `workspaces list` 查找可选 workspace。`write_default_workspace()` 保留配置文件中既有 `[cli]`、`[database]` 和其他字段，只更新 `workspace.id`，在 `workspace_name` 为 `None` 时删除旧的 `workspace.name`。

预期验证结果：配置中只有 `cli.http_base_url` 时 workspace 命令配置可加载；配置中只有 `server.host` 和 `server.port` 时 workspace 命令配置可加载；缺少可用后端地址时报错；写入默认 workspace 后 TOML 中其他字段不变；写入空名称时旧名称被移除。
...
预期验证结果：有可用后端地址时命令调用 `/workspaces` 并输出表格；`--json` 可被 `json.loads()` 解析；配置缺少 backend URL 时命令失败；缺少默认 workspace 时仍可列出 workspace。
...
实现说明：文档必须说明后端地址只来自配置，不会使用默认 URL；`set-default` 会写入 `~/.zembra/config.cli.toml`。Markdown 自然段保持连续段落，不在段落内部硬换行。
...
验证记录：2026-06-26 已运行 `uv run pytest tests/test_config.py tests/test_http_client.py tests/test_cli.py -q`，96 个测试通过。

验证记录：2026-06-26 已运行 `uv run pytest -q`，162 个测试通过。

验证记录：2026-06-26 已运行 `uv run ruff check .`，检查通过。

验证记录：2026-06-26 根据配置级联读取反馈修复 workspace 命令读取 `.zembra.env` 中 `[server].host` 和 `[server].port` 的问题；已运行 `uv run pytest tests/test_config.py tests/test_cli.py -q`，88 个测试通过。

验证记录：2026-06-26 已运行 `uv run pytest -q`，165 个测试通过。
